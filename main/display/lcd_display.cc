...
void LcdDisplay::SetupUI() {
    ...
    // Set the content of the status bar to be vertically centered
    ...
    lv_obj_set_style_margin_left(battery_label_, lvgl_theme->spacing(2), 0); // Add a left margin to separate it from the previous element
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // Collapse system messages (if it is a system message, check whether the last message is also a system message)
    ...
    if (strcmp(role, "system") == 0) {
        if (child_count > 0) {
            ...
            if (last_container != nullptr && lv_obj_get_child_cnt(last_container) > 0) {
                // Get the bubbles inside the container

                lv_obj_t* last_bubble = lv_obj_get_child(last_container, 0);
                if (last_bubble != nullptr) {
                    // Check whether the bubble type is system message
    ...
    // Make sure the text width is no less than the minimum width
    ...
    if (strcmp(role, "user") == 0) {
        ...
        // Set custom attribute mark bubble type
    ...
    if (strcmp(role, "user") == 0) {
        ...
        // Center align bubbles in container
...
void LcdDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    ...
    LvglImage* raw_image = image.release(); // Release ownership of a smart pointer

    lv_obj_add_event_cb(preview_image, [](lv_event_t* e) {
        LvglImage* img = (LvglImage*)lv_event_get_user_data(e);
        if (img != nullptr) {
            delete img; // Correctly free memory by deleting LvglImage object
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_flex_align(content_, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_SPACE_EVENLY); // Sub-objects are centered and distributed equidistantly
...
#if CONFIG_USE_WECHAT_MESSAGE_STYLE
    // In Wechat message style, if emotion is neutral, it will not be displayed.
...
void LcdDisplay::SetTheme(Theme* theme) {
    ...
    for (uint32_t i = 0; i < child_count; i++) {
        ...
        // If it's a container (user or system message), get its child objects as bubbles
        // If it is a bubble (assistant message), use it directly

        if (lv_obj_get_child_cnt(obj) > 0) {
            // May be a container, check if it is a user or system message container
            ...
            if (bg_opa == LV_OPA_TRANSP) {
                ...
                // This may be the assistant message's bubble itself

                bubble = obj;
            }
        } else {
            // No child elements, may be other UI elements, skip
        ...
        // Use saved user data to identify bubble types
