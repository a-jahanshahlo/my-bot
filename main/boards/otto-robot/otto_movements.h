...
public:
    ...
    void HandsUp(int period = 1000, int dir = 0);      // hands raised

    void HandsDown(int period = 1000, int dir = 0);    // hands down

    void HandWave(int dir = LEFT);  // wave

    void Windmill(float steps = 10, int period = 500, int amplitude = 90);  // Big windmill

    void Takeoff(float steps = 5, int period = 300, int amplitude = 40);   // take off

    void Fitness(float steps = 5, int period = 1000, int amplitude = 25);  // fitness

    void Greeting(int dir = LEFT, float steps = 5);  // greet

    void Shy(int dir = LEFT, float steps = 5);  // shy

    void RadioCalisthenics();  // radio gymnastics

    void MagicCircle();  // The magic of love goes round and round

    void Showcase();  // Display actions (concatenate multiple actions)
...
private:
    ...
    bool has_hands_;  // Is there a hand servo?
