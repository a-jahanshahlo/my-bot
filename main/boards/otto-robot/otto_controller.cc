                                ...
                                if (cJSON_IsObject(action_item)) {
                                    ...
                                    if (cJSON_IsObject(osc_item)) {
                                        ...
                                        if (cJSON_IsNumber(period_item)) {
                                            ...
                                            if (period > 3000) period = 3000;  // As described, limited to 3000 milliseconds
                                        ...
                                        if (cJSON_IsNumber(steps_item)) {
                                            ...
                                            if (steps > 20.0) steps = 20.0;  // As described, limit 20.0
                                        ...
                                        // Safety check: Prevent left and right legs from making large movements at the same time

                                        const int LARGE_MOVEMENT_THRESHOLD = 40;  // Large motion threshold: 40 degrees
                                        ...
                                        if (left_leg_large && right_leg_large) {
                                            ESP_LOGW(TAG, "Detecting large movements of the left and right legs at the same time, restricting the movement of the right leg");
                                            // Keep your right leg in its original position

                                            servo_target[RIGHT_LEG] = current_positions[RIGHT_LEG];
                                        }
                                        if (left_foot_large && right_foot_large) {
                                            ESP_LOGW(TAG, "Detecting large movements of the left and right feet at the same time, limiting the movement of the right foot");
                                        ...
                                        if (cJSON_IsNumber(speed_item)) {
                                            ...
                                            if (speed < 100) speed = 100;  // Minimum 100 milliseconds

                                            if (speed > 3000) speed = 3000;  // Maximum 3000 milliseconds
...
public:
    OttoController() {
        ...
        ESP_LOGI(TAG, "Otto robot initializes %s hand servo", has_hands_ ? "bring" : "Without");
...
void InitializeOttoController() {
    if (g_otto_controller == nullptr) {
        ...
        ESP_LOGI(TAG, "The Otto controller has been initialized and registered with the mcp tool");
    }
}
