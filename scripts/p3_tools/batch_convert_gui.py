...
class AudioConverterApp:
    ...
    def create_widgets(self):
        ...
        file_frame = ttk.LabelFrame(self.master, text="input file")
        file_frame.grid(row=2, column=0, padx=10, pady=5, sticky="nsew")
        
        # File operation button

        ttk.Button(file_frame, text="Select file", command=self.select_files,
                  width=12).grid(row=0, column=0, padx=5, pady=2)
        ttk.Button(file_frame, text="Remove selection", command=self.remove_selected,
    ...
    def select_files(self):
        file_types = [
            ("audio file", "*.wav *.mp3 *.ogg *.flac") if self.mode.get() == "audio_to_p3" 
            else ("P3 file", "*.p3")
        ]
